	...
	if (request.getParameter("action").equals("retrieve")){ // retrieve option
		//***** page retrieve section *****//
			...
			tString += "<lDesc>"+lDesc.toString().trim()+"</lDesc>";	
				...
				//for each each popup window
					...
					//tString +="<pWinNum>"+session.getAttribute("winnum"+pageNum+"_"+l)+"</pWinNum>";
		...
		tString = "<pageInfo>";
			...
			tString += "<pagenum>"+tPageCount+"</pagenum>";
				...
				//URLEncoder.encode(session.getAttribute("lDesc"+i).toString().trim());
					tString += "</lDesc"+i+">";
					tString += "<lNum"+i+">";
		...
		//*** DB Deletion ***//
		qString = "delete from sh_layout where layId = "+ lId;
		...
		//*** DB insertion (1st round: into sh_layout) ***//
		qString = "insert into sh_layout values (layout_seq.nextval, '"+lDesc+"',"+wCount+","+pwCount+","+session.getAttribute("userId")+")";
		qString2 = "select layId from sh_layout where layName = '" +lDesc+ "'";
		int layId = saveInDb(qString,qString2,"layId");
		...
		session.setAttribute("lDesc"+pCounter,lDesc);
			for(int i=0;i<wCount;i++){
				...
				//*** DB insertion (2nd round: into sh_wins) ***//
				qString = "insert into sh_wins values (wins_seq.nextval,";
			...
			for(int j=0;j<pwCount;j++){ //do it for popup windows
				...
				//*** DB insertion (3rd round: into sh_wins) ***//
		...
		String tCount = pCounter;
			...
			qString = "select userId from sh_users where userName = '"+userName+"'";
			userId = userNameCheck(qString);	
			tString = "";
			//if (userId==0) {
			//	qString = "insert into sh_users values (user_seq.nextval, '"+userName+"','')";
			//	qString2 = "select userId from sh_users where username = '" +userName+ "'";
			//	userId = saveInDb(qString,qString2,"userId");
			//} else { //if user exists, look for saved layout information...
				List lList = new ArrayList();
				lList = dbRetrieval("select * from sh_layout where userId = "+userId,"sh_layout");
				...
				for (int j=0;j<lList.size();j++){ //for each page
					...
					List wList = dbRetrieval("select * from sh_wins where layId = "+lTemp[0]+" and popId = -1 ","sh_wins");
					...
					for(int l=0;l<Integer.parseInt(lTemp[3].trim());l++){
						List wList2 = dbRetrieval("select * from sh_wins where layId = "+lTemp[0]+" and popId = "+l,"sh_wins");
						...
						for(int k=0;k<wList2.size();k++){//for each window in main window
							//System.out.println(wList2.get(k).toString());
			...
			if (userId==0) {
				...
				for (int j=0;j<lList.size();j++){ //for each page
					...
					session.setAttribute("lDesc"+(j+1),lTemp[1]);
					session.setAttribute("layId"+(j+1),lTemp[0]);
					//System.out.println("wCount"+(j+1)+", "+"lDesc"+(j+1));
...
private List dbRetrieval(String qString,String tblName){
	...
	try{
		...
		rs = stmt.executeQuery(qString);
			while(rs.next()){
				if (tblName.equals("sh_layout")){
					rList.add(rs.getInt("layId")+", "+rs.getString("layName")+", "+rs.getInt("layWinCount")+", "+rs.getInt("layPopCount"));
				} else if (tblName.equals("sh_wins")){
					rList.add(rs.getInt("winId")+", "+rs.getString("winName")+", "+rs.getString("winUrl")+", "+rs.getInt("winWidth")+", "+rs.getInt("winHeight")+", "+rs.getInt("winX")+", "+rs.getInt("winY")+", "+rs.getInt("winZ")+", "+rs.getString("winState")+", "+rs.getString("winBC"));
				}
			}
			rs.close();
			stmt.close();
