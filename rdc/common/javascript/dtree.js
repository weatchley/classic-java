...
| Updated: 17.04.2003                               |
|                                                   |
| Updated: Dec 1, 2006 by WEA                       |
|--------------------------------------------------*/

// Node object
function Node(id, pid, name, url, title, target, icon, iconOpen, open, unid) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
function dTree(objName) {
	this.config = {
		...
		inOrder					: false,
		imgPath					: '/common/javascript/img' // added by wea
	}
	this.icon = {
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, open, unid) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target, icon, iconOpen, open, unid);
};
...
// toggle Open/close a nodes (added by wea)
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		if (node.unid) str += ' unid="' + node.unid + '"';
		...
		str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node"' + ((node.unid) ? 'unid=' + node.unid : '') + '>';
	str += node.name;
