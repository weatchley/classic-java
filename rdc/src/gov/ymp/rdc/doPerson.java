...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            ...
            Domains dom = new Domains(domain, myConn);
            String pass = request.getParameter("pass");
            String newpass = request.getParameter("newpass");
            boolean isValid = per.authenticate(pass, domain, myConn, "rdc");
            if (isValid) {
                per.getLocalUser().changePassword(newpass);
                per.getLocalUser().save(myConn);
                success = true;
                outLine = "Password changed for " + per.getUserName();
            } else {
                outLine = "Password not changed, invalid old password entered.";
            }
    ...
    //    ALog.logError(userID, "csi", 0, "Permission error: '" + outLine + "'");
    //    //log(outLine);
    //}

    catch (Exception e) {
        outLine = outLine + "Exception caught: " + e.getMessage();
        ALog.logError(userID, "csi", 0, "Misc error: '" + outLine + "'");
        //log(outLine);
    }
    finally {
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    if (success) {
        ...
        out.println("alert('Error saving person!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
