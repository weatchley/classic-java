...
public class doCCategory extends HttpServlet {
...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            CCategory item = new CCategory();
            item.setDescription(description);
//System.out.println("Got Here 1" +"- command: " + command + " id: " + id + ", Description: " + item.getDescription());
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ALog.logActivity(userID, "rpms", 1, "CCategory " + item.getID() + "/\"" + HtmlUtils.getDisplayString(item.getDescription(), 100) + "\" added");

        } else if (command.equals("update")) {
            CCategory item = new CCategory(Integer.parseInt(id), myConn);
            item.setDescription(description);
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ...
            success = true;
            outLine = "CCategory " + temp + " Removed";
        ...
        ALog.logError(userID, "csi", 0, "CCategory error: '" + outLine + "'");
	    //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    out.println("<title>Citation Category</title>");
    out.println("</head>");
    ...
    if (success) {
        ...
        out.println("alert('Error processing requirement!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
