...
public class doRetentionPeriod extends HttpServlet {
...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            RetentionPeriod item = new RetentionPeriod();
            item.setDescription(description);
//System.out.println("Got Here 1" +"- command: " + command + " id: " + id + ", Description: " + item.getDescription());
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ALog.logActivity(userID, "rpms", 1, "RetentionPeriod " + item.getID() + "/\"" + HtmlUtils.getDisplayString(item.getDescription(), 100) + "\" added");

        } else if (command.equals("update")) {
            RetentionPeriod item = new RetentionPeriod(Integer.parseInt(id), myConn);
            item.setDescription(description);
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ...
            success = true;
            outLine = "RetentionPeriod " + temp + " Removed";
        ...
        ALog.logError(userID, "csi", 0, "RetentionPeriod error: '" + outLine + "'");
	    //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    if (success) {
        ...
        out.println("alert('Error processing Retention Period!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
