...
public class doFOrganization extends HttpServlet {
...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            FOrganization item = new FOrganization();
            item.setDescription(description);
//System.out.println("Got Here 1" +"- command: " + command + " id: " + id + ", Description: " + item.getDescription());
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ALog.logActivity(userID, "rpms", 1, "Functional Organization " + item.getID() + "/\"" + HtmlUtils.getDisplayString(item.getDescription(), 100) + "\" added");

        } else if (command.equals("update")) {
            FOrganization item = new FOrganization(Integer.parseInt(id), myConn);
            item.setDescription(description);
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ALog.logActivity(userID, "rpms", 2, item.getID() + "/\"" + HtmlUtils.getDisplayString(item.getDescription(), 100) + "\" updated");
            ...
            success = true;
            outLine = "Functional Organization " + temp + " Removed";
        ...
        ALog.logError(userID, "csi", 0, "FOrganization error: '" + outLine + "'");
	    //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    out.println("<title>Functional Organization</title>");
    out.println("</head>");
    ...
    if (success) {
        ...
        out.println("alert('Error processing Functional Organization!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
