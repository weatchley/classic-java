...
public class doCutoffList extends HttpServlet {
...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            CutoffList item = new CutoffList();
            item.setDescription(description);
//System.out.println("Got Here 1" +"- command: " + command + " id: " + id + ", Description: " + item.getDescription());
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ALog.logActivity(userID, "rpms", 1, "CutoffList " + item.getID() + "/\"" + HtmlUtils.getDisplayString(item.getDescription(), 100) + "\" added");

        } else if (command.equals("update")) {
            CutoffList item = new CutoffList(Integer.parseInt(id), myConn);
            item.setDescription(description);
            item.save(myConn, userID);
            success = true;
            outLine = "";
            ...
            success = true;
            outLine = "CutoffList " + temp + " Removed";
        ...
        ALog.logError(userID, "csi", 0, "CutoffList error: '" + outLine + "'");
	    //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    out.println("<title>Cutoff List</title>");
    out.println("</head>");
    ...
    if (success) {
        ...
        out.println("alert('Error processing cutoff list!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
