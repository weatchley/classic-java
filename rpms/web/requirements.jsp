...
Requirement [] list = Requirement.getItemList(myConn);
...
<script language="javascript">
  ...
  <h3>Requirement Management Page</h3><br>
...
//System.out.println(".jsp - Got Here 1" + " command: " + command);
    ...
    for (int i=0; i<list.length; i++) {
        ...
        if (doDisp) {
            ...
            <% 
            if (isAuthenticated && isAdmin && list[i].getID() != 1) {
    ...
    if (dispCount == 0) {
        %>
        <tr><td>No Requirements Available</td></tr>
        <%
    }
    %></table><%
}
...
<script language=javascript><!--
    ...
    function doNew() {
        ...
        submitForm("requirements.jsp","new");
    }
    ...
    function doSave() {
        if (isblank(document.main.description.value) || document.main.description.value.length > <%= maxLen%>) {
            ...
            submitFormResults("doRequirement",<%= ((command.equals("new")) ? "'add'" : "'update'") %>);
        }
    }
    ...
    function doDrop(id) {
        ...
        if (confirm('Delete Requirement "' + message + '"?')) {
            submitFormResults("doRequirement","drop");
        }
    }
...
<input type=hidden name=nextscript value="requirements.jsp">
<table border=0>
