...
CutoffList [] list = CutoffList.getItemList(myConn);
...
<script language="javascript">
  ...
  <h3>Cutoff List Management Page</h3><br>
...
//System.out.println(".jsp - Got Here 1" + " command: " + command);
    ...
    if (dispCount == 0) {
        %>
        <tr><td>No Cutoff List Available</td></tr>
        <%
    }
    %></table><%
}
...
<script language=javascript><!--
    ...
    function doSave() {
        if (isblank(document.main.description.value) || document.main.description.value.length > <%= maxLen%>) {
            ...
            submitFormResults("doCutoffList",<%= ((command.equals("new")) ? "'add'" : "'update'") %>);
        }
    }
    ...
    function doDrop(id) {
        ...
        if (confirm('Delete CutoffList "' + message + '"?')) {
            submitFormResults("doCutoffList","drop");
        }
    }
...
<input type=hidden name=nextscript value="cutoffList.jsp">
<table border=0>
