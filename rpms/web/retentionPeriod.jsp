...
RetentionPeriod [] list = RetentionPeriod.getItemList(myConn);
...
<script language="javascript">
  ...
  <h3>Retention Period Management Page</h3><br>
...
//System.out.println(".jsp - Got Here 1" + " command: " + command);
    ...
    if (dispCount == 0) {
        %>
        <tr><td>No Retention Periods Available</td></tr>
        <%
    }
    %></table><%
}
...
<script language=javascript><!--
    ...
    function doNew() {
        ...
        submitForm("retentionPeriod.jsp","new");
    }
    ...
    function doSave() {
        if (isblank(document.main.description.value) || document.main.description.value.length > <%= maxLen%>) {
            ...
            submitFormResults("doRetentionPeriod",<%= ((command.equals("new")) ? "'add'" : "'update'") %>);
        }
    }
    ...
    function doDrop(id) {
        ...
        if (confirm('Delete RetentionPeriod "' + message + '"?')) {
            submitFormResults("doRetentionPeriod","drop");
        }
    }
...
<input type=hidden name=nextscript value="retentionPeriod.jsp">
<table border=0>
