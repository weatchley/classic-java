...
FOrganization [] list = FOrganization.getItemList(myConn);
...
<script language="javascript">
  ...
  <h3>Functional Organization Management Page</h3><br>
...
//System.out.println(".jsp - Got Here 1" + " command: " + command);
    ...
    if (dispCount == 0) {
        %>
        <tr><td>No Functional Organizations Available</td></tr>
        <%
    }
    %></table><%
}
...
<script language=javascript><!--
    ...
    function doNew() {
        ...
        submitForm("fOrganizations.jsp","new");
    }
    ...
    function doSave() {
        if (isblank(document.main.description.value) || document.main.description.value.length > <%= maxLen%>) {
            ...
            submitFormResults("doFOrganization",<%= ((command.equals("new")) ? "'add'" : "'update'") %>);
        }
    }
    ...
    function doDrop(id) {
        ...
        if (confirm('Delete Functional Organization "' + message + '"?')) {
            submitFormResults("doFOrganization","drop");
        }
    }
...
<input type=hidden name=nextscript value="fOrganizations.jsp">
<table border=0>
...
<tr><td colspan=2><textarea id=description name=description cols=80 rows=3></textarea></td></tr>
