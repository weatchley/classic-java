...
public class doPermissions extends HttpServlet {
  ...
  private String processRequest(HttpServletRequest request, HttpServletResponse response) {
    ...
    long systemID = Long.parseLong(systemIDs);
    ...
    nextScript = (nextScript != null && nextScript.compareTo(" ") > 0) ? nextScript : "permissions.jsp";
...
//    inputstring = (inputstring != null && inputstring.compareTo(" ") > 0) ? inputstring : "";
    ...
    try {
        ...
        if (command.equals("add")) {
            ...
            Permission per = new Permission(myConn, Long.parseLong(id));
            per.setSystemID(systemID);
            per.setKey(key);
            per.setDescription(description);
            ...
            Permission per = new Permission(myConn, Long.parseLong(id));
            per.drop(myConn, userID);
            success = true;
            Sys sys = new Sys(myConn, per.getSystemID());
            outLine = "Permission " + sys.getAcronym() + "/" + per.getDescription() + " Removed";
