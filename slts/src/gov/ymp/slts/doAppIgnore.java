...
public class doAppIgnore extends HttpServlet {
...
//System.out.println("doAppIgnore - Got Here 1" +"- command: " + command + ", itemCount: " + itemCount);


    String outLine = "";
    //String nextScript = "ignoredApps.jsp";
    ...
    try {
        ...
        if (command.equals("update")) {
            ...
            // process applist
            String tempText = "";
            AppsIgnore.empty(myConn);
            if (appList != null) {
                for (int i=0; i<appList.length; i++) {
                    AppsIgnore item = new AppsIgnore();
            ...
            success = true;
            outLine = "";
            ALog.logActivity(userID, "slts", 2, "Ignored Apps updated.");
        ...
        ALog.logError(userID, "slts", 0, "doAppIgnore error: '" + outLine + "'");
        //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    if (success) {
        ...
        out.println("alert('Error processing AppIgnore!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
