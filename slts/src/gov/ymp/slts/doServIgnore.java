...
public class doServIgnore extends HttpServlet {
...
//System.out.println("doServIgnore - Got Here 1" +"- command: " + command + ", itemCount: " + itemCount);


    String outLine = "";
    //String nextScript = "ignoredServices.jsp";
    ...
    try {
        ...
        if (command.equals("update")) {
            ...
            // process servlist
            String tempText = "";
            ServiceIgnore.empty(myConn);
            if (servList != null) {
                for (int i=0; i<servList.length; i++) {
                    ServiceIgnore item = new ServiceIgnore();
            ...
            success = true;
            outLine = "";
            ALog.logActivity(userID, "slts", 2, "Ignored Services updated.");
        ...
        ALog.logError(userID, "slts", 0, "doServIgnore error: '" + outLine + "'");
        //log(outLine);
    }
  ...
  private void generateResponse(String outLine, String command, String nextScript, boolean success, HttpServletResponse response)
    ...
    out.println("<title>ServIgnore</title>");
    out.println("</head>");
    ...
    if (success) {
        ...
        out.println("alert('Error processing ServIgnore!');\n");
    }
    out.println("//-->\n");
    out.println("</script>\n");
    out.println("");
